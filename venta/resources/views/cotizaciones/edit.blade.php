    ...
    <div class="col-12">
      <div class="card mb-4">
        ...
        <div class="card-body px-4 pt-4 pb-2 flex items-center justify-center text-center"> 
            <form action="{{route('cotizaciones.update', $cotizacion->id)}}" method="post" novalidate>
            @method('put')
            @csrf
            ...
            <div style="margin-right: 20px" class="w-100">
                ...
                <input type="hidden" id="total_hidden" name="total" value="{{old('total', $cotizacion->total)}}">
                <input type="hidden" id="subtotal_hidden" name="subtotal" value="{{old('subtotal', $cotizacion->subtotal)}}">
                <input type="hidden" id="impuestos_hidden" name="impuestos" value="{{old('impuestos', $cotizacion->impuestos)}}">
                <input type="hidden" id="carrito_input" name="carrito" value="{{old('carrito', $cotizacion->carrito)}}">
                    ...
                    <select class="form-control mi-selector2" id="cliente_id" name="cliente_id">
                        ...
                        @foreach ($clientes as $cliente)
                            ...
                            <option value="{{ $cliente->id }}" data-imagen="{{ asset('uploads/' . $cliente->imagen) }}"
                                @if (old('cliente_id',$cotizacion->cliente_id) == $cliente->id)
                                    selected
                                @endif
                            >{{ $optionValue }}</option>
                        @endforeach
                ...
                <div class="d-flex justify-end items-center w-100 mt-4">
                    ...
                    <textarea 
                        ...
                        value="{{old('descripcion',$cotizacion->descripcion)}}"
                    >{{old('descripcion',$cotizacion->descripcion)}}</textarea>
...
<script>
    ...
    @if($productosAsociados)
        carrito = {!! json_encode($productosAsociados) !!};
    @endif
