        ...
        <div class="col-12">
          <div class="card mb-4">
            <div class="card-header pb-0 mb-4 ">
                <div class="row">
                    <div class="text-left mb-2 col-md-6">
                        Detalle de Cotizacion: {{$cotizacion->referencia}}
                    </div>
            ...
            <div class="my-4 flex justify-end space-x-2" style="display: flex;justify-content: end;margin-right: 30px;">
                <button onclick="exportToPDF('detalle_cotizacion')" style="background-color: #e34444" class="btn mr-2 inline-block px-2 py-1 rounded-lg font-bold text-sm text-white bg-red-600 hover:bg-red-700 transition-colors">
                ...
                <button onclick="exportToExcel('detalle_cotizacion')" style="background-color: #3d8d3d" class="btn inline-block px-2 py-1 rounded-lg font-bold text-sm text-white bg-green-600 hover:bg-green-700 transition-colors">
                    ...
                    <div class="text-center mb-5 px-4 ">
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                Empresa: {{$cotizacion->cliente->empresa}}<br>
                                @if($cotizacion->cliente->telefono)
                                Telefono: {{$cotizacion->cliente->telefono}}<br>
                                @endif
                                Email: {{$cotizacion->cliente->email}}<br>
                                Dirección: @if($cotizacion->cliente->direccion) {{$cotizacion->cliente->direccion}}@endif,  @if($cotizacion->cliente->ciudad) {{$cotizacion->cliente->ciudad}}@endif,  @if($cotizacion->cliente->pais) {{$cotizacion->cliente->pais}}@endif.<br>
                            </div>
                            <div class="col-md-4">
                                <h3 class="text-primary">Datos del Vendedor</h3>
                                Nombre: {{$cotizacion->usuario->nombre}}<br>
                                Empresa: {{$cotizacion->usuario->empresa}}<br>
                                @if($cotizacion->usuario->telefono)
                                Telefono: {{$cotizacion->usuario->telefono}}<br>
                                @endif
                                Email: {{$cotizacion->usuario->email}}<br>
                            </div>
                            <div class="col-md-4">
                                <h3 class="text-primary">Datos de la cotizacion</h3>
                                Subtotal: ${{$cotizacion->subtotal}}<br>
                                Impuestos: ${{$cotizacion->impuestos}}<br>
                                Total: ${{$cotizacion->total}}<br>
                            </div>
                        </div>
                    </div>
                    <thead>
                    ...
                    <tbody>
                        @foreach($detalles as $detalle)
                            <tr class="text-center">
                                ...
                                <td>
                                    <p class="text-xs font-weight-bold mb-0">${{$detalle->producto->precio_venta}}</p>
                                </td>
                                <td>
