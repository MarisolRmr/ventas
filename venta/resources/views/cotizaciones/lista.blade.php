        ...
        <div class="col-12">
          <div class="card mb-4">
            ...
            <div class="text-right">
                <a href="{{route('cotizaciones.create')}}" class="h-2 flex gap-2 btn btn-primary my-4 p-2 text-white hover:text-white">
            ...
            <div class="my-4 flex justify-end space-x-2" style="display: flex;justify-content: end;margin-right: 30px;">
                <button onclick="exportToPDF('cotizaciones')" style="background-color: #e34444" class="btn mr-2 inline-block px-2 py-1 rounded-lg font-bold text-sm text-white bg-red-600 hover:bg-red-700 transition-colors">
                ...
                <button onclick="exportToExcel('cotizaciones')" style="background-color: #3d8d3d" class="btn inline-block px-2 py-1 rounded-lg font-bold text-sm text-white bg-green-600 hover:bg-green-700 transition-colors">
                  ...
                  <tbody>
                    @foreach($cotizaciones as $cotizacion)
                        <tr class="text-center">
                            <td>
                                <p class="text-xs font-weight-bold mb-0">{{ $cotizacion->id }}</p>
                            </td>
                            <td>
                                <p class="text-xs font-weight-bold mb-0">{{ $cotizacion->referencia }}</p>
                            </td>
                            <td>
                                <p class="text-xs font-weight-bold mb-0">{{ $cotizacion->fecha }}</p>
                            </td>
                            <td>
                                <p class="text-xs font-weight-bold mb-0">${{ $cotizacion->total }}</p>
                            </td>
                            <td>
                                <a href="#" ><p class="text-xs font-weight-bold mb-0">{{ $cotizacion->usuario->username }}</p></a>
                            </td> 
                            

                            <td class=" px-3 py-2 exclude-column">
                                <form id="delete-form" action="{{route('cotizaciones.delete', $cotizacion->id)}}" method="POST">
                                <div style="display: flex; justify-content:center">
                                @method('delete')
                                @csrf
                                <button  id="delete-button" type="button" style="border-radius: 50px !important; border:none !important ; margin-right: 10px;" class="inline-block px-2 py-2 rounded-lg font-bold text-white bg-red-600 hover:bg-red-700 transition-colors">
                                ...
                                <a href="{{ route('cotizaciones.edit', $cotizacion->id) }}" style="border-radius: 50px !important; border:none !important ; margin-right: 10px;" class="inline-block px-2 py-2 rounded-lg font-bold text-white bg-blue-600 hover:bg-blue-700 transition-colors">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
                                <path d="M2.695 14.763l-1.262 3.154a.5.5 0 00.65.65l3.155-1.262a4 4 0 001.343-.885L17.5 5.5a2.121 2.121 0 00-3-3L3.58 13.42a4 4 0 00-.885 1.343z" />
                                </svg>
                                </a>
                                <a href="{{ route('cotizaciones.detalles', $cotizacion->id) }}" style="background-color: #a3a3a3; color: white;border-radius: 50px !important; border:none !important" class="inline-block px-2 py-2 rounded-lg font-bold text-white bg-gray-600 hover:bg-gray-700 transition-colors">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5">
...
<script>
    $(document).ready(function () {
        ...
        // Agregar evento al clic del botón
    ...
    new DataTable('#example', {
        ...
        language: {
            emptyTable: "Aún no hay cotizaciones que mostrar."
        }
    });
