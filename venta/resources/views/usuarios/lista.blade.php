        ...
        <div class="col-12">
          <div class="card mb-4">
            ...
            <div class="text-right">
                <a href="{{route('usuarios.create')}}" class="h-2 flex gap-2 btn btn-primary my-4 p-2 text-white hover:text-white">
                ...
                <table id="example" class="table align-items-center mb-0 hover">
                  <thead>
                    <tr>
                      ...
                      <th class="text-center text-uppercase  text-xxs font-weight-bolder opacity-7">Usuario</th>
                  ...
                  <tbody>
                    @foreach($usuarios as $usuario)
                        <tr class="text-center">
                            ...
                            <td>
                                @if($usuario->apellido)
                                    <p class="text-xs font-weight-bold mb-0">{{ $usuario->apellido }}</p>
                                @else
                                    <p class="text-xs font-weight-bold mb-0">Sin Apellido</p>
                                @endif
                            </td>
                            <td>
                                @if($usuario->imagen)
                                <img src="{{ asset('uploads/' . $usuario->imagen) }}" alt="Imagen de la marca" style="height: 80px; border-radius:17px">
                                @else
                            ...
                            <td>
                                @if($usuario->username)
                                    <p class="text-xs font-weight-bold mb-0">{{ $usuario->username }}</p>
                                @else
                                    <p class="text-xs font-weight-bold mb-0">Sin Usuario</p>
                                @endif
                            </td>
                            <td>
                                @if($usuario->rol)
                                ...
                                @else
                                    <p class="text-xs font-weight-bold mb-0">Sin Rol</p>
                                @endif
                            </td>
                            <td>
                                @if($usuario->status)
                                    <p class="text-xs font-weight-bold mb-0">{{ $usuario->status }}</p>
                                @else
                                    <p class="text-xs font-weight-bold mb-0">Sin Status</p>
                                @endif
                            </td>
                            <td class=" px-3 py-2 exclude-column">
                                ...
                                <a href="{{ route('usuarios.edit', $usuario->id) }}" style="border-radius: 50px !important; border:none !important" class="inline-block px-2 py-2 rounded-lg font-bold text-white bg-blue-600 hover:bg-blue-700 transition-colors">
...
<script>
    new DataTable('#example', {
        ...
        language: {
            emptyTable: "Aún no hay usuarios que mostrar."
        }
    });
