    ...
    <div class="col-12">
      <div class="card mb-4">
        ...
        <form action="{{route('categorias.update', $categoria->id)}}" method="post" novalidate>
            ...
            <div class="mb-5">
                <input type="hidden" name="imagen" id="imagen" value="{{ $categoria->imagen }}">
                @error('imagen')
                <p style="background-color: #f56565; color: #fff;margin-top: 0.5rem;border-radius: 0.5rem;font-size: 0.875rem; padding: 0.5rem; text-align: center;" class="bg-red-500 text-white my-2 rounded-lg text-sm p-2 text-center">
                        {{$message}}
                    </p>    
            ...
            <div class="text-center">
              ...
              <button type="button"class="h-2 flex gap-2 btn btn-secondary my-4 p-2 text-black hover:text-white"><a href="{{route('categorias.index')}}">Cancelar</a></button>
            </div>
          </form>
        </div>
      </div>
